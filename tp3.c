...
void __attribute__((interrupt(ipl1soft), vector(_TIMER_3_VECTOR))) Timer3Handler ( void ) {
    LATA ^= (1 << 1); // inversion de la led1
    // remise à 0 du flag
    IFS0 = IFS0 & ~(1<<14);
}

void __attribute__((interrupt(ipl1soft), vector(_TIMER_1_VECTOR))) Timer1Handler ( void ) {
    // if timer1 has expired
    ...
    displayed_7seg = (displayed_7seg + 1) & 0x3; // F : compteur modulo 4
    // F : permet de faire l'affiche sur les 4 afficheurs chacun son tour a une freq.
...
void main() {
    ...
    // reglage de la priorité
    // On choisi de reglé la priorité à 1 par default
    IPC3 |= (1<<2); // bit 4-2 : 001  
   ...
   ////////////////////////////////////////////////////////////////////////
    ...
    while(1)
    {
        ...
        // wait_timer3(); // @8 Hz 125ms
        // // inversion du bit
        // LATA ^= (1); // inversion du bit 0 avec un XOR (LD0)
        // wait_timer3();
        // LATA ^= (1);
